...
| Module | Référence | Sous-topic(s) | Champs payload |
|--------|-----------|---------------|----------------|
| **BME280** | Adafruit 2652 | `temperature`, `humidity`, `pressure` | `value` (float), `unit` (`"C"`, `"%"`, `"hPa"`), `ts` (int Unix) |
| **BH1750** | DFRobot SEN0097 | `light` | `value` (float), `unit` (`"lux"`), `ts` |
| **MPU6050** | Adafruit 3886 (STEMMA QT) | `vibration` | `x`, `y`, `z` (float, m/s²), `ts` |
| **EKMC4607112K** | SparkFun 17372 (PIR basse conso) | sous `alarm/motion` uniquement | voir §3.4 |
| Potentiomètre | — | nom selon site (voir §3.2) | `value` (float), `unit`, `ts` |
| Bouton-poussoir | — | `btn_1`, `btn_2` (numérotation indépendante du site) | `state` (`"pressed"` ou `"released"`), `ts` (int Unix) |

Les boutons sont publiés **événementiellement** sur chaque transition (front d'appui et de relâche), QoS 1, rétention non activée. Numérotation `btn_1`, `btn_2` selon le nombre de boutons câblés sur le site (voir tableau matériel), indépendamment du numéro de poste.

```
hydro-limoilou/poste-06/telemetry/btn_1   {"state": "pressed", "ts": 1739500000}
hydro-limoilou/poste-06/telemetry/btn_1   {"state": "released", "ts": 1739500001}
```
...
### 3.6 Topic `status/llm` (résumé LLM local)

Chaque appareil doit publier périodiquement un **résumé texte** généré par un LLM local (ex. Ollama + qwen2.5:3b sur la passerelle Pi 5, ou modèle embarqué sur ESP32) à partir des dernières lectures de capteurs. Ce résumé est affiché par le client central dans le panneau du site.
...
| Champ | Type | Description |
|-------|------|-------------|
| `summary` | string | Résumé langage naturel (≤ 180 caractères recommandé) |
| `model` | string | Identifiant du modèle LLM utilisé (optionnel mais recommandé pour la traçabilité) |
| `ts` | int | Timestamp Unix de génération |

Rétention **activée** (le client central doit pouvoir afficher le dernier résumé même s'il se connecte après publication).
