...
| 7 | Liligo T-Beam Supreme (LoRa) – prise en main et stack Meshtastic | Configuration LoRa/Meshtastic, topologie mesh, tests de portée; intégration capteurs. | Journal de configuration et essais de portée. | Point de contrôle d'architecture (formatif) |
| 8 | Liligo T-Beam Supreme – télémétrie et intégration serveur | Flux capteurs→LoRa→serveur, passerelle, monitoring; gestion de l'alimentation. | Itérations sur la télémétrie et le monitoring. | Revue de progression (formatif) |
| 9 | Liligo T-Beam Supreme – validation et robustesse | Tests de résilience du réseau mesh, gestion des erreurs et des mises à jour. | Ajustements finaux et documentation des essais. | **Évaluation sommative:** TP Intégration LLM et automatisation — Flux capteurs→traitement→action (20%: 15% Capacité 1 + 5% Capacité 2) |
