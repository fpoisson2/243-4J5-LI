...
1. **Phase 1:** Mise en place du Raspberry Pi comme serveur (exposition sécurisée via tunnel cloudflared), configuration des services (p. ex. courtier MQTT) et prise en main de la carte Liligo avec module cellulaire/Meshtastic et GPS; réalisation d'un premier mini-projet de logger GPS et bases de Python

2. **Phase 2:** Mise en œuvre des communications IdO (MQTT, HTTP/REST) et sécurisation des données de l'objet vers le serveur

3. **Phase 3:** Intégration des savoirs dans un cas d'usage concret, incluant l'exploration de l'apport des modèles de langage (LLM) à l'automatisation

4. **Phase 4:** Conception accélérée d'un PCB (capture schématique, DRC, génération des fichiers et soumission en fabrication) afin d'en permettre la réception à temps pour l'assemblage, puis préparation de l'architecture logicielle et matérielle

5. **Phase 5:** Réalisation d'un projet IdO complet avec assemblage/soudure, tests de bon fonctionnement, intégration finale, documentation et présentation
...
|------|--------|-----------|---------------------|-------------|
| 1 | Introduction à l'IdO et architecture: Raspberry Pi serveur (cloudflared), Liligo + GPS/Meshtastic | Rappels des acquis; configuration du Raspberry Pi comme serveur; installation du courtier MQTT et exposition via tunnel cloudflared; prise en main de la carte Liligo, configuration de Meshtastic et test du GPS; bases de Python. | Installation des outils, création des comptes nécessaires, lecture sur les architectures IdO et la sécurité d'exposition, exercices Python de base. | Diagnostic initial (formatif) |
| 2 | Logger GPS initial et interfaces de données | Lecture GPS (NMEA), horodatage, structuration JSON/CSV et journalisation locale; tests d'envoi HTTP/MQTT vers le serveur; bonnes pratiques de validation des données. | Finaliser le logger GPS v1; mise à jour du cahier de laboratoire et de la documentation technique initiale. | Mini-vérifications en laboratoire (formatif) |
| 3 | Mini-projet GPS logger v1 et conception de PCB (soumission) | Chaîne acquisition→serveur (Pi←→Liligo) et bonnes pratiques Python; capture schématique d'un PCB simple, DRC, génération des Gerbers/BOM et soumission en fabrication pour réception en milieu de session. | Finaliser schéma et fichiers de fabrication; documentation technique de base et journal de configuration. | **Évaluation sommative:** Laboratoire capteurs et Python — Mise en œuvre d'une chaîne d'acquisition sur Raspberry Pi/Liligo (15%) |
| 4 | HTTP et API REST pour la télémétrie | Conception d'endpoints d'ingestion et de consultation; sérialisation JSON; gestion des erreurs et de la latence; tests du logger via REST. | Exercices sur les appels REST (requêtes/erreurs) et intégration JSON; préparation de tokens/API. | Vérification ciblée des notions (formatif) |
| 5 | MQTT: concepts, broker et QoS | Configuration du courtier sur le serveur; publication/souscription; gestion des topics et de la persistance; ponts éventuels vers le cloud. | Préparation au quiz et essais de configuration/QoS sur le pipeline GPS. | **Évaluation sommative:** Quiz protocoles et sécurité — HTTP, MQTT, QoS, API REST et principes TLS/SSL/authentification (15%) |
| 6 | Sécurisation (TLS/SSL, authentification) et intégration bout en bout | Mise en place des certificats, authentification et politiques d'accès; pipeline capteur→réseau→serveur sécurisé; tests de charge et de robustesse. | Finalisation du TP communications et documentation des configurations (serveur, client). | **Évaluation sommative:** TP Communications MQTT/HTTP — Pipeline publication/souscription et intégration REST sécurisée (20%: 10% Capacité 1 + 10% Capacité 2) |
| 7 | Lancement du projet intégré et cadrage de l'architecture | Définition de l'architecture finale (capteurs, connectivité cellulaire/Meshtastic, services serveur); flux de données; exigences; introduction aux LLM pour l'automatisation. | Esquisse d'architecture et plan de développement (diagrammes de séquence, choix technos). | Point de contrôle d'architecture (formatif) |
| 8 | Automatisation avec LLM et chaînes de traitement | Scripts d'intégration; filtration/agrégation; géorepérage et notifications; tests systématiques. | Itérations sur l'automatisation et journal de tests. | Revue de progression (formatif) |
| 9 | Validation de l'intégration capteurs + LLM | Robustesse de la chaîne; gestion des erreurs; démonstration du cas d'usage. | Ajustements finaux et documentation du TP. | **Évaluation sommative:** TP Intégration LLM et automatisation — Flux capteurs→traitement→action (20%: 15% Capacité 1 + 5% Capacité 2) |
| 10 | Réception des PCB et contrôle qualité | Réception/inspection, revue par les pairs, plan d'assemblage, préparation BOM/stencil; tests électriques préliminaires. | Révisions de routage au besoin; préparation de l'atelier de soudure et des procédures de test. | Contrôle des livrables PCB (formatif) |
| 11 | Assemblage, soudure et tests; préparation de l'architecture finale | Montage, inspection, mise sous tension, tests fonctionnels; intégration capteurs/actionneurs et API. | Plan de tests détaillé et mise à jour de la documentation. | Dépôt formatif: schéma/PCB et architecture du projet final |
| 12 | Intégration matérielle et logicielle du projet final | Intégration des communications sécurisées, chiffrement et authentification; liaison cellulaire/Meshtastic; fiabilité et tolérance aux pannes. | Implantation des communications et journal des anomalies. | Point d'étape projet (formatif) |
| 13 | Itérations, validation et documentation technique | Tests de performance, QoS et autonomie énergétique; structuration du rapport de documentation. | Rédaction de la documentation et préparation de la démonstration. | Dépôt formatif: prototype fonctionnel et documentation préliminaire |
| 14 | Optimisation finale et préparation de la présentation | Tests finaux, corrections et répétition de la présentation. | Finition du rapport et des supports de présentation. | Répétition évaluée (formatif) |
| 15 | Présentation et remise du projet final | Démonstration, remise du rapport et rétroaction. | Aucune (fin de session) | **Évaluation sommative:** Projet final IdO avec PCB et déploiement (30%: 10% Capacité 1 + 20% Capacité 2) |
