...
Communicates with the ESP32 via MQTT (not serial).

Renders to the framebuffer via SDL2 kmsdrm, reads touch via evdev.

MQTT topics (prefix = device_id from mqtt_config.py):
  Subscribe: {id}/testscreen/status       → JSON with btn/pot/led states
  Publish:   {id}/testscreen/led/red      → "ON" / "OFF"
...
class State:
    """Shared state updated from the MQTT thread."""
    def __init__(self):
        self.btn1 = False
        self.btn2 = False
        self.pot1 = 0
...
def main():
    ...
    # ── MQTT setup ──
    client_id = f"pi-testscreen-{int(time.time())}"
    ...
    while running:
        ...
        # ── Title ──
        title = font_title.render("Component Test (MQTT)", True, WHITE)
