...
| 4 | Labo 3 – Workflow complet KiCad | Workflow complet de conception PCB: saisie de schéma dans KiCad, bibliothèques, ERC; placement/routage 2 couches, plan de masse, DRC; génération des Gerbers/BOM. | Réviser le workflow KiCad; préparer les composants du projet de mi-session. | Vérification ciblée des notions (formatif) |
| 5 | Projet de mi-session – Shield LilyGO A7670G | Travail sur le projet de mi-session: prototype breadboard (2 DELs, 2 boutons, 2 potentiomètres, accéléromètre); début de la capture schématique KiCad. | Avancer le prototype et la documentation. | Suivi de progression (formatif) |
| 6 | Projet de mi-session – Finalisation PCB | Finalisation du schéma et routage PCB; génération des fichiers de fabrication (Gerbers/BOM); documentation technique. | Finaliser les livrables du projet de mi-session. | Suivi de progression (formatif) |
